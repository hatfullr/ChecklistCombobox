...
# TODO:
#    - Check to make sure the user has control over setting border thicknesses etc. without it
#      messing up the look.
#    - Clean up the code.
...
class ChecklistCombobox(ttk.Combobox):
    ...
    def __init__(self,master=None,**kw):
        ...
        if len(self.values) > 0:
            self.checkbuttons = [None]*len(self.values)
            self.variables = [None]*len(self.checkbuttons)
            for i in range(0,len(self.checkbuttons)):
                # Carefully trick tkinter into making checkbuttons on top of the popdown listbox
                self.checkbuttons[i] = tk.Checkbutton()
                self.checkbuttons[i]._w = '%s%s' % (self.checkbutton_frame,self.checkbuttons[i])
                self.tk.eval('checkbutton %s' % (self.checkbuttons[i]))
                self.variables[i] = tk.IntVar()
        else:
            self.checkbuttons = []
            self.variables = []
        ...
        self.custom_postcommand = kw.pop('postcommand',None)
        ...
        kw['checkbutton_text'] = kw.get('checkbutton_text',self.values)
    ...
    def custom_configure(self,cnf=None,**kw):
        ...
        for key, value in kw.items():
            ...
            elif self.checkbuttons and keysplit[0] == 'checkbutton':
        ...
        for key,value in checkbutton_kw.items():
            if not isinstance(value,(list,tuple,np.ndarray)):
                ...
                raise ValueError("Array-like argument for configuring Checkbuttons is length '"+str(len(value))+"', but expected length '"+len(self.checkbuttons)+"'")
    ...
    def select(self,button):
        ...
        if button == self.checkbuttons[self.selection]: return
    ...
    def on_down(self,event=None):
        if self.selection < len(self.checkbuttons)-1: self.select(self.checkbuttons[self.selection+1])
        return "break"
    def on_up(self,event=None):
        if self.selection > 0: self.select(self.checkbuttons[self.selection-1])
        return "break"
    ...
    def on_checkbutton_click_press(self,event):
        # Send the button itself to self.select
